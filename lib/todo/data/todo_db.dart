...
//private internal constructor
  ...
  final store = intMapStoreFactory.store('todos6');
  ...
  Future<Database> get database async {
    if (_database == null) {
      await _openDb().then((db) {
        _database = db;
        insertTodo(Todo('Squat', '65'));
        insertTodo(Todo('Bench', '80'));
        insertTodo(Todo('Deadlift', '120'));
        insertTodo(Todo('Press', '65'));
      });
    }
  ...
  Future _openDb() async {
    ...
    final dbPath = join(docsPath.path, 'todos6.db');
